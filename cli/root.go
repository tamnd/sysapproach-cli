// Package cli assembles the sysapproach command tree from the sysapproach
...
// NewApp assembles the kit application from the sysapproach domain. The
...
//
// To add a command, declare it in sysapproach/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := sysapproach.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(sysapproach.Domain{}).Register(app)
