...
describes what users get. Pick a format with `-o`, or let sysapproach choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
sysapproach <command> -o table   # aligned columns for reading
sysapproach <command> -o jsonl   # one JSON object per line, for piping
sysapproach <command> -o json    # a single JSON array
sysapproach <command> -o csv     # spreadsheet friendly
sysapproach <command> -o tsv     # tab-separated
sysapproach <command> -o url     # just the URL column
sysapproach <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
sysapproach <command> --fields id,title,url
```
...
```bash
sysapproach <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
sysapproach <command>            # a table, because this is a terminal
sysapproach <command> | wc -l    # JSONL, because this is a pipe
```
